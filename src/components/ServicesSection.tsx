...
const ServicesSection = () => {
  const services = [
    ...
    },
    {
      ...
      description: "Scalable server architectures, APIs, and database solutions for your applications.",
    ...
    },
    {
      ...
      description: "Cloud infrastructure setup, deployment, and optimization for maximum performance.",
  ...
  const getColorClasses = (color: string) => {
    switch (color) {
      ...
      case 'orange':
        return 'text-orange group-hover:bg-orange group-hover:text-orange-foreground';
      case 'purple':
        return 'text-purple group-hover:bg-purple group-hover:text-purple-foreground';
      default:
        return 'text-primary group-hover:bg-primary group-hover:text-primary-foreground';
    }
  };
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card key={index} className="group hover:shadow-xl transition-all duration-500 hover:-translate-y-3 bg-card border-0 overflow-hidden relative">
              {/* Hover gradient overlay */}
              <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-accent/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              
              <CardHeader className="relative z-10">
                <div className="mb-4">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${getColorClasses(service.color)} bg-${service.color}/10`}>
